...
class Creator(RoutedAgent):
    ...
    def __init__(self, name) -> None:
        super().__init__(name)
        model_client = GeminiClient.get_gemini_chat_client(temperature=1.0)
    ...
    def get_user_prompt(self):
        ...
        with open(f"{LabConstants.PATH}/agent.py", "r", encoding="utf-8") as f:
            template = f.read()
    ...
    async def handle_my_message_type(self, message: messages.Message, ctx: MessageContext) -> messages.Message:
        ...
        with open(f'{LabConstants.PATH}/{filename}', "w", encoding="utf-8") as f:
    ...
    def _get_system_message():
        return ("You are an Agent that is able to create new AI Agents. "
                "You receive a template in the form of Python code that "
                "creates an Agent using Autogen Core and Autogen Agentchat. "
                "You should use this template to create a new Agent with "
                "a unique system message that is different from the template, "
                ...
                "and it must inherit from RoutedAgent and have an __init__ "
                "method that takes a name parameter. Also avoid environmental interests "
                "- try to mix up the business verticals so that every agent is different. "
                "Respond only with the python code, no other text, and no markdown code blocks.")
